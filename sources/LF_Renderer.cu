...
LF_Renderer::LF_Renderer(const std::string& path_LF, const std::string& path_pixelrange, const size_t& iw, const size_t& ih, const size_t& lf_length, const size_t& num_LFs, const double& dpp, const int& stride, const int& initPosX, const int& initPosY, bool use_window)
{
	...
	lfu_window = new LFU_Window(this->config, curPosX, curPosY, &state_disk_read_thread, this->use_window); // 3x3 형태의 LFU Window, 2D list
...
#if PREFETCHING_MODE != 2
	workers.push_back(std::thread(&LF_Renderer::loop_background_prefetch, this, std::ref(*LRU), std::ref(*window), slice_set, std::ref(candidates_of_future_position), stream_h2d, std::ref(mtx)));
#endif
	// H2D, LF Read를 위한 worker threads
	if (this->use_window == true) {
		workers.push_back(std::thread(&LF_Renderer::loop_background_LF_read, this, std::ref(*window), std::ref(curPosX), std::ref(curPosY), std::ref(light_field_size), std::ref(state_main_thread)));
	}
...
uint8_t* LF_Renderer::do_rendering(int& newPosX, int& newPosY)
{
	...
	if (this->use_window == false) {
		window->update_LFU(newPosX, newPosY);
	...
	while (!(getLFUID(curPosX, curPosY) == window->m_center->id && state_disk_read_thread >= DISK_READ_THREAD_CENTER_LFU_ODD_FIELD_READ_COMPLETE)) {
		printf("reading now\n");
	}
	...
	int mode = LRU->synchronize_HashmapOfPtr(state_disk_read_thread, stream_main); // Host memory <-> Device memory 동기화
	mtx.unlock();
	...
	int offset = this->config->output_width / 2 - (output_width_each_dir[0] / 2);
	cudaError_t err;
	synthesize << < blocksPerGrid_F, threadsPerBlock, 0, stream_main >> > (synthesized_view, LRU->d_devPtr_hashmap_odd, LRU->d_devPtr_hashmap_even, offset, mode, 0, curPosX, curPosY, localPosX[0], localPosY[0], (float)config->DPP, (int)config->LF_width, (int)config->LF_height, (int)config->LF_length, (int)config->slice_width);
	offset += output_width_each_dir[0];
	synthesize << < blocksPerGrid_R, threadsPerBlock, 0, stream_main >> > (synthesized_view, LRU->d_devPtr_hashmap_odd, LRU->d_devPtr_hashmap_even, offset, mode, 1, curPosX, curPosY, localPosX[1], localPosY[1], (float)config->DPP, (int)config->LF_width, (int)config->LF_height, (int)config->LF_length, (int)config->slice_width);
	offset += output_width_each_dir[1];
	synthesize << < blocksPerGrid_B, threadsPerBlock, 0, stream_main >> > (synthesized_view, LRU->d_devPtr_hashmap_odd, LRU->d_devPtr_hashmap_even, offset, mode, 2, curPosX, curPosY, localPosX[2], localPosY[2], (float)config->DPP, (int)config->LF_width, (int)config->LF_height, (int)config->LF_length, (int)config->slice_width);
	offset += output_width_each_dir[2];
	synthesize << < blocksPerGrid_L, threadsPerBlock, 0, stream_main >> > (synthesized_view, LRU->d_devPtr_hashmap_odd, LRU->d_devPtr_hashmap_even, offset, mode, 3, curPosX, curPosY, localPosX[3], localPosY[3], (float)config->DPP, (int)config->LF_width, (int)config->LF_height, (int)config->LF_length, (int)config->slice_width);
...
void LF_Renderer::loop_background_prefetch(LRU_Cache& LRU, const LFU_Window& window, SliceSet slice_set[][100], std::vector<std::pair<int, int>>& nbrPosition, cudaStream_t stream_h2d, std::mutex& mtx)
{
	while (state_main_thread != MAIN_THREAD_TERMINATED) {
		mtx.lock();
		cache_slice_in_background(LRU, window, slice_set, nbrPosition, stream_h2d);
		mtx.unlock();
	}
}

void LF_Renderer::loop_background_LF_read(LFU_Window& window, const int& curPosX, const int& curPosY, const int& light_field_size, const MAIN_THREAD_STATE& state_main_thread)
{
	while (state_main_thread != MAIN_THREAD_TERMINATED) {
...
std::pair<int, int> LF_Renderer::cache_slice(LRU_Cache& LRU, const LFU_Window& window, SliceSet slice_set[][100], const int& posX, const int& posY)
{
	...
	for (SliceSet::iterator it = slice_set[localPosX][localPosY].begin(); it != slice_set[localPosX][localPosY].end(); it++)
	{
		for (int slice_num = it->range_begin; slice_num <= it->range_end; slice_num++)
		{
			...
			if (state_disk_read_thread == DISK_READ_THREAD_CENTER_LFU_ODD_FIELD_READ_COMPLETE) {
			...
			else if (state_disk_read_thread >= DISK_READ_THREAD_CENTER_LFU_EVEN_FIELD_READ_COMPLETE) {
				int hit = LRU.put(id, window.m_center->LF[it->direction]->odd_field + slice_location, ODD);
...
int LF_Renderer::cache_slice_in_background(LRU_Cache& LRU, const LFU_Window& window, SliceSet slice_set[][100], std::vector<std::pair<int, int>>& nbrPosition, cudaStream_t stream_h2d)
{
	if (state_disk_read_thread < DISK_READ_THREAD_CENTER_LFU_ODD_FIELD_READ_COMPLETE) return -1;
	...
	while (1)
	{
		while (1) {
			for (int p = 0; p < 4; p++) {
				if (state_main_thread == MAIN_THREAD_H2D) {
				...
				if (i < slice_set[posX_at_p][posY_at_p].size())
				{
					...
					if (slice_num <= slice_set[posX_at_p][posY_at_p].at(i).range_end) {
						...
						size_t slice_location = find_slice_from_LF(id.image_number, id.slice_number);
						if (state_disk_read_thread == DISK_READ_THREAD_CENTER_LFU_ODD_FIELD_READ_COMPLETE) {
							LRU.put(id, window.m_center->LF[dir]->odd_field + slice_location, stream_h2d, ODD);
						}
						else if (state_disk_read_thread >= DISK_READ_THREAD_CENTER_LFU_EVEN_FIELD_READ_COMPLETE) {
							LRU.put(id, window.m_center->LF[dir]->odd_field + slice_location, stream_h2d, ODD);
							LRU.put(id, window.m_center->LF[dir]->even_field + slice_location, stream_h2d, EVEN);
						}
					}
				}
			}
	...
	for (int i = 0; i < nbrPosition.size(); i++)
	{
		...
		for (SliceSet::iterator it = slice_set[posX % 100][posY % 100].begin(); it != slice_set[posX % 100][posY % 100].end(); it++)
		{
			...
			for (int slice_num = it->range_begin; slice_num <= it->range_end; slice_num++)
			{
				...
				if (state_disk_read_thread == DISK_READ_THREAD_CENTER_LFU_ODD_FIELD_READ_COMPLETE) {
					LRU.put(id, window.m_center->LF[it->direction]->odd_field + slice_location, stream_h2d, ODD);
				}
				else if (state_disk_read_thread >= DISK_READ_THREAD_CENTER_LFU_EVEN_FIELD_READ_COMPLETE) {
					LRU.put(id, window.m_center->LF[it->direction]->odd_field + slice_location, stream_h2d, ODD);
					LRU.put(id, window.m_center->LF[it->direction]->even_field + slice_location, stream_h2d, EVEN);
				}
			}
		}
	}
...
__global__ void synthesize(uint8_t* outImage, uint8_t** d_hashmap_odd, uint8_t** d_hashmap_even, int offset, int mode, int direction, int posX, int posY, int localPosX, int localPosY, float dpp, int width, int height, int legnth, int slice_width, float fov, float times)
{
	...
	if (tw < output_width && th < (height >> 1)) // Thread index must not exceed output resolution
	{
		...
		int output_image_offset = offset % ow;
		...
		outImage[((2 * th) * (ow * 3) + output_image_offset * 3) + tw * 3 + 0] = oddpel_ch0; // b 
		outImage[((2 * th) * (ow * 3) + output_image_offset * 3) + tw * 3 + 1] = oddpel_ch1; // g 
		outImage[((2 * th) * (ow * 3) + output_image_offset * 3) + tw * 3 + 2] = oddpel_ch2; // r 
		...
		if (mode == 1) {
			...
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 0] = evenpel_ch0; // b 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 1] = evenpel_ch1; // g 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 2] = evenpel_ch2; // r
		}
		else
		{
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 0] = oddpel_ch0; // b 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 1] = oddpel_ch1; // g 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 2] = oddpel_ch2; // r 
		}
#if 0
		if (tw == 0) // for debug
		{
			outImage[((2 * th) * (ow * 3) + output_image_offset * 3) + tw * 3 + 0] = 0; // b 
			outImage[((2 * th) * (ow * 3) + output_image_offset * 3) + tw * 3 + 1] = 0; // g 
			outImage[((2 * th) * (ow * 3) + output_image_offset * 3) + tw * 3 + 2] = 255; // r 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 0] = 0; // b 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 1] = 0; // g 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 2] = 255; // r
		}
		if (tw == output_width - 1) // for debug
		{
			outImage[((2 * th) * (ow * 3) + output_image_offset * 3) + tw * 3 + 0] = 255; // b 
			outImage[((2 * th) * (ow * 3) + output_image_offset * 3) + tw * 3 + 1] = 0; // g 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 0] = 255; // b 
			outImage[((2 * th + 1) * (ow * 3) + output_image_offset * 3) + tw * 3 + 1] = 0; // g 
		}
#endif
	}
